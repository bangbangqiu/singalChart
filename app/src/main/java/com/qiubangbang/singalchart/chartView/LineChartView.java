...
public class LineChartView extends View {
    ...
    private void init(Context ct, AttributeSet attrs) {
        ...
        dotColor = attributes.getColor(R.styleable.DottedLine_dotColor, defaultColor);
        ...
        xRightExtend = (int) attributes.getDimension(R.styleable.DottedLine_xRightExtend, dipToPx(6));
        ...
        //设置曲线的渐变色
        gradientColor = new int[]{0xff5cd7c0, 0xff6ed7c9, 0xff69cad3, 0xff6fbae8, 0xff6fbae8, 0xff70bbe8};
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                for (int i = 0; i < xLabels.length; i++) {
                    int x = dotPoint.x + i * (xlength / (xLabels.length - 1));
...
//        //3 标尺层
    ...
    private void drawDot(Canvas canvas) {
        for (int i = 5 + internal + 1; i < points.size(); i++) {
            ...
            canvas.drawCircle(points.get(i).x, points.get(i).y, dipToPx(outSize),
                    fillPaint);

            fillPaint.setColor(Color.WHITE);
            fillPaint.setAlpha(170);
            canvas.drawCircle(points.get(i).x, points.get(i).y, dipToPx(innerSize),
                    fillPaint);
        }
    }
    ...
    private void drawCurveLine(Canvas canvas) {
        ...
        for (int i = 6 + internal + 1; i < points.size(); i++) {
            ...
            //贝塞尔曲线 控制点（目前还没怎么太了解这个东西）
            int midX = (points.get(i - 1).x + points.get(i).x) / 2;
            Point p1 = new Point(midX, y1);
            Point p2 = new Point(midX, y2);

            Path path2 = new Path();
            path2.moveTo(points.get(i - 1).x, y1);
            path2.cubicTo(p1.x, p1.y, p2.x, p2.y, points
                    .get(i).x, y2);

            path2.lineTo(points.get(i).x, points.get(0).y);
            path2.lineTo(points.get(i - 1).x, points.get(0).y);
            canvas.drawPath(path2, fillPaint);

            Path path1 = new Path();
            path1.moveTo(points.get(i - 1).x, y1);
            linePaint.setColor(curveLineColor);
            linePaint.setStrokeWidth(dipToPx(2.5f));
            linePaint.setShader(new LinearGradient(points.get(i - 1).x, y1, points.get(i).x, y2, gradientColor[i - 6 - internal - 1], gradientColor[i - 5 - internal - 1], Shader.TileMode.CLAMP));//渐变色
            Log.d("qiubit_r x:", points.get(i - 1).x + "\ty:" + y1);
            linePaint.setShadowLayer(dipToPx(1.0f), 0, 0, curveLineColor);
            path1.cubicTo(p1.x, p1.y, p2.x, p2.y, points.get(i).x
                    , y2);
    ...
    private void changePointPosition(float x, float y) {
        //为了方便点击，加大点击区域
        for (int i = 5 + internal + 1; i < points.size(); i++) {
            if (Math.abs(x - points.get(i).x) < dipToPx(15)
                    && Math.abs(y - points.get(i).y) < dipToPx(15)) {
