...
/**
 * 饼形图
 * Created by qiubangbang on 2016/1/10.
 */

public class PieChatView extends View {
    private static final int DEFAULT_MINANGLE = 30;//默认最小角度
    private int sum = 0;
    ...
    int offx = dp2px(40);
    int offy = dp2px(4);//设置提示文字的Y轴便宜量
    private boolean isShowKind = false;//是否展示描述文字
    ...
    public PieChatView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTextPaint.setTextSize(dp2px(10));//设置引线上的文字大小
...
//                    drawCenterText(mCanvas, centerTitle, 0, 0, mTextPaint);
                    //绘制数值展示,不为空才展示
                    if (!kindsMap.containsKey("default")) {
                        if (isShowKind) {
                            drawAngle = (kinds + " " + drawAngle);
                        }
                        drawValue(mCanvas, textAngle, drawAngle + "%", colors.get(i));
                    } else {
                        centerTitle = "暂无数据";
                        drawCenterText(mCanvas, centerTitle, 0, 0, mTextPaint);
                    }
    ...
    //绘制数值展示
    private void drawValue(Canvas mCanvas, float textAngle, String drawAngle, int color) {
        //根据象限箭头分左上 右上 左下 右下
        int textPadding = dp2px(2);
        Rect rect = new Rect();
        mPaint.setColor(color);
        mTextPaint.setColor(color);
        Path path = new Path();
        float x = (float) (mRadius * 1.1 * Math.cos(Math.toRadians(360 - textAngle)));
        float y = (float) (-mRadius * 1.1 * Math.sin(Math.toRadians(360 - textAngle))) + rect.height() / 2;
        mCanvas.drawCircle(x, y, dp2px(2), mPaint);
        mPaint.setStyle(Paint.Style.STROKE);
        if (textAngle >= 0 && textAngle <= 90) { //画布坐标系第一象限(数学坐标系第四象限)
            mTextPaint.setTextAlign(Paint.Align.RIGHT);
            path.moveTo(x, y);
            path.lineTo(x + offy, y + offy);
            path.lineTo(x + offx, y + offy);
            //规定特殊字符N代替换行，防止文字过长，超过区域(xxxN20%)
            if (drawAngle.contains("N")) {
                mCanvas.drawText(drawAngle.substring(0, drawAngle.indexOf("N")), x + offx, y + offy - textPadding - dp2px(12), mTextPaint);
                mCanvas.drawText(drawAngle.substring(drawAngle.indexOf("N") + 1, drawAngle.length()), x + offx, y + offy - textPadding, mTextPaint);
            } else {
                mCanvas.drawText(drawAngle, x + offx, y + offy - textPadding, mTextPaint);
            }
    ...
    private void drawCenterText(Canvas mCanvas, String text, float x, float y, Paint mPaint) {
        ...
        mCanvas.drawText(text, x, y + rect.height() / 2, mPaint);
    }
